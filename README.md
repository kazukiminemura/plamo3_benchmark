...
- `--max-seq-len`: deprecated。NPU 変換は stateful KV-cache IR を使うため、この値は無視されます
...
`--target-device NPU` を指定した場合は、int32 入力の stateful KV-cache IR を作ります。
KV cache は OpenVINO の `ReadValue` / `Assign` state として内部化されます。
...
- `input_ids`: `int32`、dynamic sequence
- `attention_mask`: `int32`、dynamic sequence
- `position_ids`: `int32`、dynamic sequence
- `beam_idx`: `int32`、shape `[1]`
- KV cache: OpenVINO state
- `fp32` 指定時も保存形式は `fp16`
- `int8`: `INT8_SYM`
- `int4`: `INT4_SYM`、`ratio=1.0`、`group_size=-1`

`--max-seq-len` は古い固定長 NPU 変換用のオプションです。現在の NPU 変換では受け取りますが無視します。
...
- NPU 向け stateful IR: int32 入力で OpenVINO InferRequest の state を reset しながら逐次生成
...
NPU 向け変換でも同じ `StatefulKV` wrapper を使い、`input_ids`、`attention_mask`、`position_ids`
を int32 にした KV-cache IR を作ります。
...
input dtype、KV-cache layout などの変換 metadata を保存します。
